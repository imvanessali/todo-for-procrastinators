...
(async function () {
  ...
  // 归档已完成任务：设为过去日期，从笔记本两页收起，仍保留在甘特图与搜索
  async function archiveTodo(t) {
    if (!t.done) return;
    const cd = t.completed_at ? isoToLocalDay(t.completed_at) : TODAY;
    const day = cd < TODAY ? cd : addDays(TODAY, -1);
    t.day = day;
    render();
    await store.update(t.id, { day });
  }
  ...
  function todoNode(t, isToday) {
    ...
    // 归档：仅完成项显示（由 CSS 控制），收起出笔记本、保留在甘特图
    const arch = el('button', 'act-archive');
    arch.innerHTML = ICON_ARCHIVE;
    arch.title = '归档（收起，仍保留在甘特图）';
    arch.onclick = () => archiveTodo(t);
